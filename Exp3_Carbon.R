#Exp3 Activated carbon
#Paper: L.t._SExt
#The dose makes the poison
#Rodolfo Ángeles, 25/11/25
#
#
#1. Load data
#2. Normality
...
data <- read.csv("data/Exp3_Carbon.csv")
data_summary <- data %>%
...
boxplot_graph <- ggplot(data_summary,
  ...
  labs(title = "Biomass by Extraction and Concentration with activated carbon",
       x = "Treatment (Activated Carbon : Soil Extract : Dose)",
       y = "Biomass (g/L)") +
  # Estilos del tema
  theme_bw() +
...
write.csv(as.data.frame(normality_tests), "out/Exp3_Shapiro_results.csv", row.names = TRUE)
# Guardar ANOVA
print(anova_summary)
write.csv(as.data.frame(anova_summary[[1]]), "out/Exp3_Anova_results.csv", row.names = TRUE)

# Guardar prueba de Tukey
write.csv(as.data.frame(tukey_result$Treatment), "out/Exp3_Tukey_results.csv", row.names = TRUE)

# Guardar tabla resumen
write.csv(summary_table, "out/Exp3_Summary_table.csv", row.names = FALSE)

# Guardar la gráfica
ggsave("out/Exp3_Boxplot.pdf", boxplot_graph, width = 8, height = 6)
