#Exp4 Filtration vs autoclave
#Paper: L.t._SExt
#The dose makes the poison
#Rodolfo Ángeles, 27/11/25
#
#
#1. Load data
#2. Normality
...
data <- read.csv("data/Exp4_Filtr.csv")
data_summary <- data %>%
  mutate(Treatment = interaction(Detox,Ext,Dose, sep = ":"))
...
cld_table <- cld_table %>%
  separate(Treatment, into = c("Detox", "Ext", "Dose"), sep = ":")
...
summary_table <- data_summary %>%
  group_by(Detox, Ext, Dose) %>%
  summarise(
    Mean_X = mean(X, na.rm = TRUE),
    SD_X = sd(X, na.rm = TRUE),
    Median_X = median(X, na.rm = TRUE)) %>%
  left_join(cld_table, by = c("Detox", "Ext", "Dose")) %>%
...
boxplot_graph <- ggplot(data_summary,
  ...
  scale_linetype_manual(values = c("solid", "twodash", "dashed"), # Líneas para pH
                        name = "Detoxification",
                        labels = c("No", "Carbon", "Filtration")) +
  scale_shape_manual(values = c(1, 0, 2, 5, 6), # Forma de los puntos para Dose
                     name = "Dose",
                     labels = levels(data_summary$Dose)) +
  # Etiquetas del gráfico
  labs(title = "Biomass by Extraction, Concentration and detoxification",
       x = "Treatment (Detoxification : Soil Extract : Dose)",
       y = "Biomass (g/L)") +
  # Estilos del tema
  theme_bw() +
...
write.csv(as.data.frame(normality_tests), "out/Exp4_Shapiro_results.csv", row.names = TRUE)
# Guardar ANOVA
print(anova_summary)
write.csv(as.data.frame(anova_summary[[1]]), "out/Exp4_Anova_results.csv", row.names = TRUE)

# Guardar prueba de Tukey
write.csv(as.data.frame(tukey_result$Treatment), "out/Exp4_Tukey_results.csv", row.names = TRUE)

# Guardar tabla resumen
write.csv(summary_table, "out/Exp4_Summary_table.csv", row.names = FALSE)

# Guardar la gráfica
ggsave("out/Exp4_Boxplot.pdf", boxplot_graph, width = 8, height = 6)
